...
function helpPanel(){
	echo -e "\n${yellowColour}[*]${endColour}${blueColour} Uso ./WifiTigo.sh${endColour}"
	echo -e "\t${blueColour}-a${endColour}${yellowColour} Modo de ataque${endColour}"
	echo -e "\t\t${redColour}Handshake${endColour}"
	echo -e "\t\t${redColour}PKMID${endColour}"
	echo -e "\t${blueColour}-n${endColour}${yellowColour} Nombre de la interfaz de red${endColour}"
	echo -e "\t${blueColour}-h${endColour}${yellowColour} Panel de Ayuda${endColour}\n"
	exit 0
}
...
function startAttack(){
		...
		ifconfig ${networkCard}mon up; killall dhclient wpa_suplicant 2>/dev/null
